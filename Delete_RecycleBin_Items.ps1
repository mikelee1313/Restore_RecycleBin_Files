<#
.SYNOPSIS
    Permanently deletes items from SharePoint Online recycle bins across multiple sites using batch processing.

.DESCRIPTION
    This script connects to each SharePoint Online site listed in a provided text file and permanently deletes items deleted within the 
    ...
    Items are deleted in batches of up to 200 items per API call to avoid throttling issues. 
    The script logs detailed information about each step, including successful deletions, throttling events, and errors.
...
.OUTPUTS
    Generates a log file in the user's TEMP directory named "Delete_RecycleBin_Item_<timestamp>.txt" containing detailed execution logs.

.NOTES
    - Requires PnP.PowerShell module.
    - Ensure the Azure AD application has appropriate permissions to access and permanently delete items from SharePoint recycle bins.
    - Handles throttling responses (HTTP 429) by respecting the "Retry-After" header.

.EXAMPLE
    ./Delete_RecycleBin_Items.ps1

    Executes the script using predefined variables and permanently deletes recently deleted items from the recycle bins of sites listed in "C:\temp\SiteList_DeleteItems.txt".
#>
...
$batchSize = 50  # Number of items to permanently delete per API call
...
$logFilePath = "$env:TEMP\Delete_RecycleBin_Item_$startime.txt"
...
foreach ($SiteURL in $Sites) {
    ...
    # Batch delete using configured batch size
    ...
    while ($leftToProcess -gt 0) {
        ...
        Write-Info "Building statement to permanently delete the following $numToProcess files"
        $body = "{""ids"":["
        ...
        $body += "]}"
        Write-Info "Performing API Call to permanently delete items from RecycleBin..."
